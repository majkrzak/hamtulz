...
import Data.Log.Model (Connection, Document, Location, Metadata, Operator, Program, Record, Report, Station, Stations, Via)
...
makeLensesWith (lensRules & lensField .~ (\_ _ name -> [TopName (mkName $ nameBase name)])) ''Via
